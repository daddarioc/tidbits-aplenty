...
import com.daddarioc.tidbits.service.CategoryQueryService;
import com.daddarioc.tidbits.service.CategoryService;
...
public class CategoryResourceIntTest {
    ...
    @Test
    @Transactional
    public void getAllCategoriesByTidbitsIsEqualToSomething() throws Exception {
        // Initialize the database
        Tidbit tidbit = TidbitResourceIntTest.createEntity(em);
        em.persist(tidbit);
        em.flush();
        category.addTidbit(tidbit);
        categoryRepository.saveAndFlush(category);
        Long tidbitsId = tidbit.getId();

        // Get all the categoryList where tidbits equals to tidbitsId
        defaultCategoryShouldBeFound("tidbitsId.equals=" + tidbitsId);

        // Get all the categoryList where tidbits equals to tidbitsId + 1
        defaultCategoryShouldNotBeFound("tidbitsId.equals=" + (tidbitsId + 1));
    }
